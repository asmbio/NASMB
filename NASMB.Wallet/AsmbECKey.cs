...
namespace NASMB.Wallet
{
    public class AsmbECKey
    {
        ...
        public AsmbECKey(string privateKey)
        {
            _ecKey = new ECKey(privateKey.HexToByteArray(), true);
        }


        public AsmbECKey(byte[] vch, bool isPrivate)
        {
            _ecKey = new ECKey(vch, isPrivate);
        }

        public AsmbECKey(byte[] vch, bool isPrivate, byte prefix)
        {
            _ecKey = new ECKey(ByteUtil.Merge(new[] { prefix }, vch), isPrivate);
        }

        internal AsmbECKey(ECKey ecKey)
        {
            _ecKey = ecKey;
        }


        public byte[] CalculateCommonSecret(AsmbECKey publicKey)
        {
        ...
        public static AsmbECKey GenerateKey(byte[] seed = null)
        {
            ...
            if (privateBytes.Length != 32)
                return GenerateKey();
            return new AsmbECKey(privateBytes, true);
        }

        public static AsmbECKey GenerateKey()
        {
            var gen = new ECKeyPairGenerator("EC");
            var keyGenParam = new KeyGenerationParameters(SecureRandom, 256);
            gen.Init(keyGenParam);
        ...
        {
            var key = new AsmbECKey(privateKey.HexToByteArray(), true);
        ...
        public static AsmbECKey RecoverFromSignature(AsmbECDSASignature signature, byte[] hash)
        {
            return new AsmbECKey(ECKey.RecoverFromSignature(GetRecIdFromV(signature.V), signature.ECDSASignature, hash,
                false));
        }

        public static AsmbECKey RecoverFromParityYSignature(AsmbECDSASignature signature, byte[] hash)
        {
            return new AsmbECKey(ECKey.RecoverFromSignature(signature.V.ToIntFromRLPDecoded(), signature.ECDSASignature, hash,
                false));
        }

        public static AsmbECKey RecoverFromSignature(AsmbECDSASignature signature, int recId, byte[] hash)
        {
            return new AsmbECKey(ECKey.RecoverFromSignature(recId, signature.ECDSASignature, hash, false));
        }

        public static AsmbECKey RecoverFromSignature(AsmbECDSASignature signature, byte[] hash, BigInteger chainId)
        {
            return new AsmbECKey(ECKey.RecoverFromSignature(GetRecIdFromVChain(signature.V, chainId),
                signature.ECDSASignature, hash, false));
        }

        public AsmbECDSASignature SignAndCalculateV(byte[] hash, BigInteger chainId)
        {
#if NETCOREAPP3_1 || NET5_0_OR_GREATER
            if (SignRecoverable)
            {
                ...
                return AsmbECDSASignatureFactory.FromComponents(r.ToBytes(), s.ToBytes(),
            ...
            var signature = _ecKey.Sign(hash);
                ...
                return new AsmbECDSASignature(signature);
        ...
        public AsmbECDSASignature SignAndCalculateYParityV(byte[] hash)
        {
#if NETCOREAPP3_1 || NET5_0_OR_GREATER
            if (SignRecoverable)
            {
                ...
                privKey.TrySignRecoverable(hash, out var recSignature);
                recSignature.Deconstruct(out Scalar r, out var s, out var recId);
                return AsmbECDSASignatureFactory.FromComponents(r.ToBytes(), s.ToBytes(), new[] { (byte)(recId) });
            }
            else
            {
#endif
        ...
        public AsmbECDSASignature SignAndCalculateV(byte[] hash)
        {
#if NETCOREAPP3_1 || NET5_0_OR_GREATER
            if (SignRecoverable)
            {
        ...
        public AsmbECDSASignature Sign(byte[] hash)
        {
            var signature = _ecKey.Sign(hash);
            return new AsmbECDSASignature(signature);
        }

        public bool Verify(byte[] hash, AsmbECDSASignature sig)
        {
            return _ecKey.Verify(hash, sig.ECDSASignature);
        }

        public bool VerifyAllowingOnlyLowS(byte[] hash, AsmbECDSASignature sig)
        {
